...
else
{
    ...
    foreach($frames as $name => $values){
        if($values[1] == "https://crmdev.fmpco.com/521/521.php"){
            $customerTab[$name] = $values;
        }else{
    ...
    {
        if($i == 1){
            foreach($customerTab as $tab){
                 if (is_array($tab)) {
                       $xtpl->assign("MODULE_NAME", $tab[4]);
                        $xtpl->assign("MODULE_KEY", $tab[3]);


                        if($tab[3] == $currentModule && (!empty($_REQUEST['record']) and $_REQUEST['record']==$tab[0] and !empty($_REQUEST['tab'])))
                        {
                            $tabClass = "currentTab";
                            $otherTab = "currentTab";
                            $xtpl->assign("TAB_CLASS", "currentTab");
                            $xtpl->assign("OTHER_TAB", "currentTab");
                            $xtpl->assign('HEAD_TITLE', $tab[4] . ' - ');
                        }
                        else
                        {
                            $tabClass = "otherTab";
                        ...
                        $xtpl->assign("LINK", '<a   class="'.$tabClass.'Link"  href="index.php?module='.$tab[3].'&action=index&record='.$tab[0].'&tab='.$tab[2].'">'.$tab[4].'</A>');
            ...
            if($module_name == $currentModule && ($module_name != 'iFrames' || empty($_REQUEST['record'])|| (!empty($_REQUEST['tab']) && $_REQUEST['tab']=='false')))
            {
                ...
                $xtpl->assign("TAB_CLASS", "currentTab");
                $xtpl->assign("OTHER_TAB", "currentTab");
                $xtpl->assign('HEAD_TITLE', $app_list_strings['moduleList'][$module_name] .' - ');
            }
            else
            {
                $tabClass = "otherTab";
            ...
            if($module_name[3] == $currentModule && (!empty($_REQUEST['record']) and $_REQUEST['record']==$module_name[0] and !empty($_REQUEST['tab'])))
            {
                ...
                $xtpl->assign("TAB_CLASS", "currentTab");
                $xtpl->assign("OTHER_TAB", "currentTab");
                $xtpl->assign('HEAD_TITLE', $module_name[4] . ' - ');
            }
            else
            {
                $tabClass = "otherTab";
