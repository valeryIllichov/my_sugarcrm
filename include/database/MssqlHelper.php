...
EOSQL;
        ...
        ) 
    {
        ...
        $columns = array();
        while (($row=$this->db->fetchByAssoc($result)) !=null) {
            $columns[strtolower($row['COLUMN_NAME'])]['name']=strtolower($row['COLUMN_NAME']);
            $columns[strtolower($row['COLUMN_NAME'])]['type']=strtolower($row['TYPE_NAME']);
            if ( $row['TYPE_NAME'] == 'decimal' ) {
                $columns[strtolower($row['COLUMN_NAME'])]['len']=strtolower($row['PRECISION']);
                $columns[strtolower($row['COLUMN_NAME'])]['len'].=','.strtolower($row['SCALE']);
            }
            elseif ( in_array($row['TYPE_NAME'],array('nchar','nvarchar')) )
                $columns[strtolower($row['COLUMN_NAME'])]['len']=strtolower($row['PRECISION']);
            elseif ( !in_array($row['TYPE_NAME'],array('datetime','text')) )
                $columns[strtolower($row['COLUMN_NAME'])]['len']=strtolower($row['LENGTH']);
            if ( stristr($row['TYPE_NAME'],'identity') ) {
                $columns[strtolower($row['COLUMN_NAME'])]['auto_increment'] = '1';
                $columns[strtolower($row['COLUMN_NAME'])]['type']=str_replace(' identity','',strtolower($row['TYPE_NAME']));
            }
            /*
            if ($row['Null'] == 'NO' && !stristr($row['Key'],'PRI'))
                $columns[strtolower($row['COLUMN_NAME'])]['required'] = 'true';
            */
            if ( !is_null($row['COLUMN_DEF']) ) {
                $matches = array();
                $row['COLUMN_DEF'] = html_entity_decode($row['COLUMN_DEF'],ENT_QUOTES);
                if ( preg_match("/\([\(|'](.*)[\)|']\)/i",$row['COLUMN_DEF'],$matches) )
                    $columns[strtolower($row['COLUMN_NAME'])]['default'] = $matches[1];
                elseif ( preg_match("/\(N'(.*)'\)/i",$row['COLUMN_DEF'],$matches) )
                    $columns[strtolower($row['COLUMN_NAME'])]['default'] = $matches[1];
                else
                    $columns[strtolower($row['COLUMN_NAME'])]['default'] = $row['COLUMN_DEF'];
            }
        }
        return $columns;
    }
