 ...
 class SearchForm extends EditView{
    ...
    var $showSavedSearchesOptions = true;
 	...
 	function display($header = true){
    	global $theme, $timedate, $image_path;
 		$header_txt = '';
 		$footer_txt = '';
 		$return_txt = '';
        ...
        $this->th->ss->assign('IMG_PATH', $image_path);
		...
		$groupLayout = $_SESSION['groupLayout'];
		/* END - SECURITY GROUPS */ 
	        ...
	        foreach($this->seed->toArray() as $name => $value) {
	            ...
	            if(isset($this->fieldDefs[$name.'_'.$this->parsedView]['function'])) {
	            	$this->fieldDefs[$name.'_'.$this->parsedView]['type']='multienum';
	       	 		...
	       	 		if(is_array($this->fieldDefs[$name.'_'.$this->parsedView]['function']) && $name != 'custno_c') {
                ...
                if($field_value != '') {
                    ...
                    foreach ($parms['db_field'] as $db_field) {
						if (strstr($db_field, '.') === false) {
                        	...
                        	/** BEGIN - SECURITY GROUPS / RELATEDDATA */
                        	/** [IC] related data search; add check for relateddata */
                        	if ($type == 'relateddata' && !empty($this->seed->field_name_map[$field]['link'])
                        		&& !empty($this->seed->field_name_map[$field]['rname'])) {
                        			$link = $this->seed->field_name_map[$field]['link'];
                        			if (($this->seed->load_relationship($link))){
                        				$db_field = $this->seed->$link->getRelatedTableName() .(!empty($this->seed->field_name_map[$field]['custom_table'])?"_cstm":"") .  '.' . $this->seed->field_name_map[$field]['rname'];
                        			} else {
                        				//Best Guess for table name if we can't load the relationship
                        				$db_field = strtolower($link['module']) . (!empty($this->seed->field_name_map[$field]['custom_table'])?"_cstm":"") . '.' . $db_field;
                        			}
                        	} else 
                        	/** END SECURITY GROUPS / RELATEDDATA */
