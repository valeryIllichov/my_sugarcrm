...
//            echo'<pre>'; print_r($dataSet);echo'</pre>';
	...
	function processData(){
		$data = array();
//                echo'<pre>'; print_r($this->data_set);echo'</pre>';
...
//                echo'<pre>'; print_r($this->group_by[1]);echo'</pre>';
//                echo'<pre>'; print_r($this->data_set);echo'</pre>';
...
//                echo'<pre>'; print_r($new_data);echo'</pre>';
		foreach ($new_data as $groupByKey => $value){
			...
			//$label = $this->is_currency ? ($this->currency_symbol . $amount . 'K') : $amount; // Base format
                        $label = $this->is_currency ? ($this->currency_symbol . number_format($amount,0,'.',',')) : $amount; //FMP format
...
//                        echo'<pre>';print_r($drill_down);echo'</pre>';
...
//                                                      echo'<pre>';print_r($drill_down_param);echo'</pre>';
						}
						...
						if($this->is_currency) {
						  $sub_amount = $this->formatNumber($this->convertCurrency($new_data[$groupByKey][$i]['total']));
                                                  ...
                                                  $sub_amount_formatted = $this->currency_symbol . number_format($sub_amount,0,'.',',') ;// . 'K'; FMP format
						} else {
...
//                echo'<pre>'; print_r($this->super_set);echo'</pre>';
		return $data;
	}
...
//        echo'<pre>';print_r($translate1);echo'</pre>';
