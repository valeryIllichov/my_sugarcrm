...
{var inline=1;if(typeof(force_load)=='undefined')
{force_load=false;}
...
{var module=get_module_name();var id=get_record_id();var layout_def_key=get_layout_def_key();url='index.php?sugar_body_only=1&module='+module+'&subpanel='+child_field+'&action=SubPanelViewer&inline='+inline+'&record='+id+'&layout_def_key='+layout_def_key;}
...
current_child_field=child_field;}
if(typeof(url)!='undefined'&&url.indexOf('refresh_page=1')>0){document.location.reload();}}
...
url='index.php?module=Home&action=SaveSubpanelLayout&layout='+order+'&layoutModule='+currentModule;if(typeof SUGAR.subpanelUtils.currentSubpanelGroup!='undefined'){url=url+'&layoutGroup='+SUGAR.subpanelUtils.currentSubpanelGroup;}
var cObj=YAHOO.util.Connect.asyncRequest('GET',url,{success:success,failure:success});},inlineSave:function(theForm,subpanel){ajaxStatus.showStatus(SUGAR.language.get('app_strings','LBL_SAVING'));var success=function(data){if(subpanel=='projects')subpanel='project';SUGAR.subpanelUtils.cancelCreate('subpanel_'+subpanel);var module=get_module_name();var id=get_record_id();var layout_def_key=get_layout_def_key();try{eval('result = '+data.responseText);}catch(err){}
...
var reloadpage=false;if(subpanel=='activities'&&document.getElementById(theForm).status[document.getElementById(theForm).status.selectedIndex].value=='Held'){reloadpage=true;}
YAHOO.util.Connect.setForm(theForm,true,true);var cObj=YAHOO.util.Connect.asyncRequest('POST','index.php',{success:success,failure:success,upload:success});return false;},sendAndRetrieve:function(theForm,theDiv,loadingStr,subpanel){function success(data){theDivObj=document.getElementById(theDiv);subpanelContents[theDiv]=new Array();subpanelContents[theDiv]['list']=theDivObj;subpanelContents[theDiv]['newDiv']=document.createElement('div');subpanelContents[theDiv]['newDiv'].innerHTML=data.responseText;theDivObj.style.display='none';theDivObj.parentNode.insertBefore(subpanelContents[theDiv]['newDiv'],theDivObj);if(isIE||isSafari||1)SUGAR.util.evalScript(data.responseText);subpanelLocked[theDiv]=false;setTimeout('enableQS(false)',500);ajaxStatus.hideStatus();}
