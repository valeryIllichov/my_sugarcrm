...
class SugarFolder {
	...
	function getCountItems ( $folderId ) {
		...
		if ($this->is_dynamic) {
	    	...
	    	$modified_select_query = preg_replace($pattern, $replacement, $this->generateSugarsDynamicFolderQuery());
	...
	function getUserFolders(&$rootNode, $folderStates, $user=null, $forRefresh=false) {
		...
		foreach($folders as $a) {
			$label = ($a['name'] == 'My Email' ? $mod_strings['LBL_LIST_TITLE_MY_INBOX'] : $a['name']);
