...
$query .= 					"LEFT JOIN contacts
	...
	function get_list_view_data(){
		...
		if ($this->status != "Completed" && $this->status != "Deferred" ) {
			$task_fields['SET_COMPLETE'] = "<a href='index.php?return_module=$currentModule&return_action=$action&return_id=" . ((!empty($focus->id)) ? $focus->id : "") . "&action=EditView&module=Tasks&record={$this->id}&status=Completed'>".get_image($image_path."close_inline","title=".translate('LBL_LIST_CLOSE','Tasks')." border='0'")."</a>";
		}
