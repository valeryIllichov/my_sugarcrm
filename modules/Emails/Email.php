...
class Email extends SugarBean {
	...
	function fill_in_additional_parent_fields() {
		...
		elseif (!empty($beanList[$this->parent_type])) {
			...
			if (is_subclass_of($parent, 'Person')&& $this->parent_type != 'Users') {
				$query = "SELECT first_name, last_name, assigned_user_id parent_name_owner from $parent->table_name where id = '$this->parent_id'";
			} else if($this->parent_type == 'Users') {
				$query = "SELECT first_name, last_name, created_by parent_name_owner from $parent->table_name where id = '$this->parent_id'";
			} else {
				$query = "SELECT name ";
