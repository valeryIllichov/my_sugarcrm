...
class ViewLayoutView extends ViewEdit
{
    ...
    function display ($preview = false)
    {
        ...
        {
            $smarty->assign ( $image, get_image ( $image_path . $file, '' ) ) ;
        }
        ...
        } else
        {
            ...
            if (! $this->fromModuleBuilder)
            {
                $buttons [] = array ( 'id' => 'saveBtn' , 'text' => translate ( 'LBL_BTN_SAVE' ) , 'actionScript' => "onclick='Studio2.handleSave();'" ) ;
                $buttons [] = array ( 'id' => 'publishBtn' , 'text' => translate ( 'LBL_BTN_SAVEPUBLISH' ) , 'actionScript' => "onclick='Studio2.handlePublish();'" ) ;
            ...
            {
                $buttons [] = array ( 'id' => 'saveBtn' , 'text' => $GLOBALS [ 'mod_strings' ] [ 'LBL_BTN_SAVE' ] , 'actionScript' => "onclick='Studio2.handlePublish();'" ) ;
                $buttons [] = array ( 'id' => 'historyBtn' , 'text' => translate ( 'LBL_HISTORY' ) , 'actionScript' => "onclick='ModuleBuilder.history.browse(\"{$this->editModule}\", \"{$this->editLayout}\")'" ) ;
            }
        }
        ...
        if ($this->fromModuleBuilder)
        {
            ...
            $ajax->addCrumb ( $this->package, 'ModuleBuilder.getContent("module=ModuleBuilder&action=package&package=' . $_REQUEST [ 'view_package' ] . '")' ) ;
            ...
            $ajax->addCrumb ( translate ( 'LBL_LAYOUTS', 'ModuleBuilder' ), 'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=wizard&view_module=' . $this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")' ) ;
            $ajax->addCrumb ( $translatedViewType, 'ModuleBuilder.getContent("module=ModuleBuilder&action=editLayout&view=' . $this->editLayout . '&view_module=' . $this->editModule . '")' ) ;
        } else
        {
        ...
        $ajax->addSection ( 'center', translate ( 'LBL_EDIT_LAYOUT', 'ModuleBuilder' ), $smarty->fetch ( 'modules/ModuleBuilder/tpls/layoutView.tpl' ) ) ;
        if ($preview) {
