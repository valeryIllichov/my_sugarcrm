 ...
 */
 ...
 class ViewSearchView extends ViewListView
 {
 	...
 	function constructAjax ()
 	{
 		...
 		$searchLabel = translate ( 'LBL_' . strtoupper ( $this->editLayout), 'ModuleBuilder' ) ;
 		if ($this->fromModuleBuilder)
 		{
 			...
 			'ModuleBuilder.getContent("module=ModuleBuilder&action=module&view_package=' . $_REQUEST [ 'view_package' ] . "&view_module={$this->editModule}" . '")'  ) ;
 			$ajax->addCrumb ( translate ( 'LBL_LAYOUTS', 'ModuleBuilder' ),
 			'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=wizard&view_module=' . $this->editModule
 			. '&view_package=' . $_REQUEST['view_package'] . '")'  ) ;
 			$ajax->addCrumb ( translate ( 'LBL_SEARCH_FORMS', 'ModuleBuilder' ),
 			'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=wizard&search=1&view_module=' .
 			$this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")'  ) ;
 			$ajax->addCrumb ( $searchLabel, 'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=editLayout&view='
 			. $this->editLayout . '&view_module=' . $this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")' ) ;
 		} else
 		{
 			...
 			$ajax->addCrumb ( translate ( 'LBL_LAYOUTS', 'ModuleBuilder' ), 
 				'ModuleBuilder.getContent("module=ModuleBuilder&action=wizard&layouts=1&view_module=' . $this->editModule . '")'  ) ;
			/* BEGIN - SECURITY GROUPS */ 
			global $groupName;
			$ajax->addCrumb ( translate ( $groupName ), '' ) ;
			/* END - SECURITY GROUPS */ 
 			$ajax->addCrumb ( $searchLabel, 
 				'ModuleBuilder.getContent("module=ModuleBuilder&action=editLayout&view=' . $this->editLayout . '&view_module=' . $this->editModule . '")'  ) ;
 		}
 		return $ajax ;
 	}
