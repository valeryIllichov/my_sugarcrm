...
<input type='hidden' name='subpanel' value='{$submodule}'>
...
<table id="content">
<tr><td colspan='100'>{$buttons}</td></tr><tr>
<td>
...
<div id={$label} style="float: left; border: 1px gray solid; padding:4px; margin-right:4px; margin-top: 8px; width:{$groupWidth}%;">
<h3 >{$label}</h3>
<ul id='ul{$slotCounter}'>
    ...
    <table width='100%'>
        <tr>
            ...
            <td align="right">
                ...
                onclick="var value_label = document.getElementById('subslot{$modCounter}label').innerHTML; var value_width = document.getElementById('subslot{$modCounter}width').innerHTML; ModuleBuilder.getContent('module=ModuleBuilder&action=editProperty&view_module={$view_module}{if isset($subpanel)}&{$subpanel}{/if}{if $MB}&MB={$MB}&view_package={$view_package}{/if}&id_label=subslot{$modCounter}label&name_label=label_{if isset($value.label)}{$value.label}{else}{$key}{/if}&title_label={$MOD.LBL_LABEL_TITLE}&value_label=' + value_label + '&id_width=subslot{$modCounter}width&name_width={$MOD.LBL_WIDTH}&value_width=' + value_width );">
            </td>
            </tr>
            <tr class='fieldValue'>
