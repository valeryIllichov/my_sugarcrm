...
<tr>
    <td class="dataField"><input name='{$prefix}date_closed' onblur="parseDate(this, '$cal_dateformat');" size='12' maxlength='10' id='${prefix}jscal_field' type="text" value="">&nbsp;<img src="themes/default/images/jscalendar.gif" alt="{$app_strings['LBL_ENTER_DATE']}"  id="${prefix}jscal_trigger" align="absmiddle"></td>
</tr>
EOQ;
if($showaccount){
	$the_form .= <<<EOQ
...
<tr>
    <td class="dataLabel">$lbl_probability</td>
</tr>
<tr>
    <td class="dataField"><input type="text" title="" value="" id="probability" name="{$prefix}probability"/>$probability_script</td>
</tr>
<tr>
...
$the_form .= <<<EOQ
		...
		<input name='{$prefix}date_closed' size='12' maxlength='10' id='{$prefix}jscal_field' type="text" value=""> <img src="themes/default/images/jscalendar.gif" alt="{$app_strings['LBL_ENTER_DATE']}"  id="jscal_trigger" align="absmiddle"><br>
...
$the_form .= <<<EOQ
		...
		<script type="text/javascript">
		Calendar.setup ({
			inputField : "{$prefix}jscal_field", daFormat : "$cal_dateformat", ifFormat : "$cal_dateformat", showsTime : false, button : "jscal_trigger", singleClick : true, step : 1
		});
		</script>
EOQ;
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (isset($GLOBALS['check_notify'])) {
		$check_notify = $GLOBALS['check_notify'];
                   ...
                   if(isset($_GET['multiopp'])){
                       session_start();
                       if(isset($_GET['lastopp']) && $check_notify){
                          $check_notify = TRUE;
                          $focus->multiopp_notify = TRUE;
                       }else{
                           $check_notify = FALSE;
                       }
                       if($GLOBALS['check_notify']){
                            if(isset($_GET['firstopp'])){
                                unset($_SESSION["multiopp"]);
                                unset($_SESSION["notify_list"]);
                            }
                           $notify_list = $focus->get_notification_recipients();
                            foreach ($notify_list as $notify_user)
                            {
                                    $focus->notify_user = $notify_user;
                                    $_SESSION["notify_list"][$notify_user->user_name] = $notify_user;
                            }
                            if(isset($_GET['lastopp'])){
                                 $_SESSION["multiopp"][] = serialize($focus);
                            }
                       }
                   }
	...
	$return_id = $focus->id;
                  if(isset($_GET['multiopp']) && $GLOBALS['check_notify']){
                           $_SESSION["multiopp"][] = serialize($focus);
                  }
