...
class Opportunity extends SugarBean {
	...
	var $module_dir = "Opportunities";
                //multicreate opportunities notify
                  var $multiopp_notify = false;
                  var $notify_user;
...
$query .= 			"LEFT JOIN users
		...
		if($show_deleted == 0){	    
			...
			AND opportunities.deleted=0";
                ...
                if($where != "" && strstr($where, '|GROUP BY')){
                   $group = explode("|", $where); 
                   $groupby = $group[1];
                   $where = $group[0];
                }
	...
	*/
	...
	function set_notification_body($xtpl, $oppty)
	{
		global $app_list_strings;
		global $sugar_config;
                                    if($oppty->multiopp_notify){
                                        $body = "";
                                        if(isset($_SESSION["multiopp"])){
                                            foreach($_SESSION["multiopp"] as $opp){
                                                $multi_opp = unserialize($opp);
                                                if($multi_opp->notify_user->user_name == $oppty->notify_user->user_name){
                                                    $parsedSiteUrl = parse_url($sugar_config['site_url']);
                                                    $host = ($parsedSiteUrl['host'] != $httpHost && !empty($httpHost)) ? $httpHost : $parsedSiteUrl['host'];
                                                    ...
                                                    $opp_id = isset($multi_opp->id) ? $multi_opp->id : $oppty->id;
                                                    $URL= $cleanUrl."/index.php?module={$multi_opp->module_dir}&action=DetailView&record={$opp_id}";
                                                    
                                                    $query = "SELECT acc.custno_c custno from accounts  acc where acc.id = '" . $multi_opp->account_id . "'";
                                                    $result = $this->db->query($query,true," Error filling in additional detail fields: ");
                                                    $row = $this->db->fetchByAssoc($result);
                                                    $body .= "   Opportunity Name: ".$multi_opp->name.
                                                                       "\n   Customer Account: ".$multi_opp->account_name." (".$row['custno'].")".
                                                                       "\n   Estimated Annualized Sales: ".$multi_opp->amount.
                                                                       "\n   Expected Close Date: ".$multi_opp->date_closed.
                                                                       "\n   Sales Stage: ".(isset($multi_opp->sales_stage)?$app_list_strings['sales_stage_dom'][$multi_opp->sales_stage]:"").
                                                                       "\n   Additional Opportunities Information: ".$multi_opp->description.
                                                                       "\n   You may review this Opportunity at: \n<".$URL.">\n\n";
                                                }
                                            }
                                            $xtpl->assign("MULTIOPP_BODY", $body);
                                        }
                                    }else{
                                        $query = "SELECT acc.custno_c custno from accounts  acc where acc.id = '" . $oppty->account_id . "'";
                                        $result = $this->db->query($query,true," Error filling in additional detail fields: ");
                                        $row = $this->db->fetchByAssoc($result);
                                        $xtpl->assign("OPPORTUNITY_NAME", $oppty->name);
                                        $xtpl->assign("OPPORTUNITY_CUSTOMER", $oppty->account_name." (".$row['custno'].")");
