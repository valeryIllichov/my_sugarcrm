...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function search($ieId, $subject='', $from='', $to='', $body='', $dateFrom='', $dateTo='') {
		...
		foreach($beans as $bean) {
			...
			$group = ($bean->group_id != $current_user->id) ? 'group.' : '';
		...
		if ($this->protocol == 'pop3') {
			if($this->pop3_open()) {
				...
				if (preg_match("/OK/", $buf) > 0) {
					...
					if (trim($mailserverResponse[2]) == $messageId) {
						$foundMessageNo = true;
					}
				} //if
	...
	function importOneEmail($msgNo, $uid, $forDisplay=false, $clean_email=true) {
		...
		if($forDisplay || $dupeCheckResult) {
			...
			if(!empty($header->ccaddress)) {
				$email->cc_addrs_names	 = $this->handleTranserEncoding($this->handleMimeHeaderDecode($header->ccaddress), $structure->encoding);
			} // if
...
EOQ;
			...
			if($this->attachmentCount > 0) {
				...
				for($i=0; $i<$theCount; $i++) {
					...
					$tempName = $this->tempAttachment[$name];
