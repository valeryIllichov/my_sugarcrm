...
//		$this->assigned_user_name = get_assigned_user_name($this->assigned_user_id);
		...
		$query .= "
                                cases.*,accounts.name as account_name,
...
//				$query .= " FROM cases";
	...
	function fill_in_additional_detail_fields()
	{
		...
		$this->account_name = $account_info['account_name'];
		$this->account_id = $account_info['account_id'];
		$this->subject_c = $account_info['subject_c'];
		$this->account_number = $account_info['custno_c'];
	}
	...
	*/
	...
	function get_list_view_data(){
		...
		$temp_array['SET_COMPLETE'] =  "<a href='index.php?return_module=Home&return_action=index&action=EditView&module=Cases&record=$this->id&status=Closed'>".get_image($image_path."close_inline","title=".translate('LBL_LIST_CLOSE','Cases')." border='0'")."</a>";
	...
	*/
	...
	{
		global $app_list_strings;
//		$xtpl->assign("CASE_SUBJECT", (isset($case->subject_c) ? $app_list_strings['subject_c'][$case->subject_c]:""));


		$xtpl->assign("CASE_SUBJECT", $case->account_name . " (" . $case->custno_c .  ")");
		$xtpl->assign("CASE_PRIORITY", (isset($case->priority) ? $app_list_strings['case_priority_dom'][$case->priority]:""));
		$xtpl->assign("CASE_ACCOUNT_NAME", $case->account_name);
	...
	function getAccount($case_id){
		...
		$query = "SELECT acc.id, acc.name, acc.custno_c, cases.subject_c from accounts  acc, cases  where acc.id = cases.account_id and cases.id = '" . $case_id . "' and cases.deleted=0 and acc.deleted=0";
		...
		if($row != null){
			...
			$ret_array['subject_c']        = $row['subject_c'];
			$ret_array['custno_c']        = $row['custno_c'];
