...
if (!$is_scheduler) {
	...
	require_once($theme_path.'layout_utils.php');
        ...
        $settings_button = '';
        $settings_button .= "<div style='float:right; text-align: right; width: 50%; font-size: 12px;'>";
        $settings_button .= "<a href='#' onClick='$(\"#settings_dialog\").dialog(\"open\");'>Settings</a>";
        $settings_button .= "&nbsp;&nbsp;&nbsp;&nbsp;</div>";
        $settings_button .= "<div style='clear: both;'></div>";

        $selectedValue = array(1000000=>'never', 5=>'5 days', 10=>'10 days', 20=>'20 days', 30=>'30 days');
        if($current_user->getPreference('ORPersonalSettings') !== null){
            $selectedOR = $current_user->getPreference('ORPersonalSettings');
        }else{
            $selectedOR = 5*24*60*60;
        }

        if($current_user->getPreference('SASRPersonalSettings') !== null){
            $selectedSASR = $current_user->getPreference('SASRPersonalSettings');
        }else{
            $selectedSASR = 30*24*60*60;
        }

        function createSelect($arr, $report, $selectValue){
            $selecForView = '<select style="display: inline" id="'.$report.'PersonalSettings">';
            foreach ($arr as $key=>$val){
                $selecForView .= '<option value="'.$key.'"';
                if($key == $selectValue)$selecForView .= ' selected="selected"';
                $selecForView .= '>'.$val.'</option>';
            }
            $selecForView .= '</select>';
            return $selecForView;
        }
	...
	$xtpl->assign("IMAGE_PATH", $image_path);
        $xtpl->assign("SETTINGS_SELECTION_HEADER", $settings_button);
        $xtpl->assign("SELECTLIST_SASR", createSelect($selectedValue, 'SASR', $selectedSASR/24/60/60));
        $xtpl->assign("SELECTLIST_OR", createSelect($selectedValue, 'OR', $selectedOR/24/60/60));
        $xtpl->assign("SELECTED_OR", $selectedOR/24/60/60);
        $xtpl->assign("SELECTED_SASR", $selectedSASR/24/60/60);
