...
class Calendar2ViewAjaxSplitSave extends SugarView {

 	function Calendar2ViewAjaxSplitSave(){
 		parent::SugarView();
 	...
 	function display() {
		...
		if($_REQUEST['cur_module'] == 'Meetings'){
			...
			$jn = "cal2_meeting_id_c";
                ...
                $mix_date = $_REQUEST['split_date'].' '.date('h:ia',strtotime($_REQUEST['date_start']));
                $split_date_unix = to_timestamp_from_uf($_REQUEST['split_date']);
                $start_date_unix = to_timestamp_from_uf($_REQUEST['date_start']);
                if ($start_date_unix < $split_date_unix){$start_date = $mix_date; } else {$start_date = $_REQUEST['date_start'];}

                $r_days = "";
		$r_type = "";

                //$GLOBALS['log']->test('view.ajaxsplitsave.php for  start_date before weekly check='. $start_date);
		if (isset($_REQUEST['cal2_repeat_days_c']) && !empty($_REQUEST['cal2_repeat_days_c'])) $r_days = $_REQUEST['cal2_repeat_days_c'];

		if (isset($_REQUEST['cal2_repeat_type_c']) && !empty($_REQUEST['cal2_repeat_type_c'])) $r_type = $_REQUEST['cal2_repeat_type_c'];
                if(!empty($r_type) && (!isset($_REQUEST['cal2_repeat_end_date_c']) || !empty($_REQUEST['cal2_repeat_end_date_c'])) ){
                    if(!empty($r_days) && $r_type == 'Weekly'){
                        $st_unix = to_timestamp_from_uf($start_date);
                        $st_day = date("w",$st_unix - date('Z',$st_unix)) + 1;
                        $start_week_start_day = $st_unix - ($st_day -1)*60*60*24;
                        //$GLOBALS['log']->test('view.ajaxsplitsave.php weekly: start_week_start_day='.timestamp_to_user_formated2($start_week_start_day));
                        for($d = 1; $d < 8; $d++)
                             if(strpos($r_days,(string)($d)) !== false){

                                    $start=$start_week_start_day + ($d-1)*60*60*24;
                                    $start_date = timestamp_to_user_formated2($start);
                                    //$GLOBALS['log']->test('view.ajaxsplitsave.php start_day in weekly type='.timestamp_to_user_formated2($start));
                                            break;
                ...
                //$GLOBALS['log']->test('view.ajaxsplitsave.php for  start_date='. $start_date);
...
//		}
		...
		$q = ''
                                           ...
                                           . 'description  '
                                           . 'FROM ' . $table_name . ' '
                                           . 'WHERE id = \'' . $_REQUEST['clicked_record'] . '\' '
