	...
	if($m % 3 == 0)
		echo "<tr>";
			...
			echo '<td class="yearCalBodyMonth" align="center" valign="top" scope="row">';
				echo '<a href="index.php?module='.$currentModule.'&action=index&view=month&&hour=0&day=1&month='.($m+1).'&year='.timestamp_to_user_formated2($month_start_unix,'Y').'">'.$GLOBALS['app_list_strings']['dom_cal_month_long'][$m+1].'</a>';
						...
						echo '<tr class="monthViewDayHeight yearViewDayHeight">';
							for($d = 0; $d < 7; $d++){
								...
								if($curr_time < $month_start_unix || $curr_time >= $month_end_unix)
									...
									$monC = '<a href="index.php?module='.$currentModule.'&action=index&view=day&hour=0&day='.timestamp_to_user_formated2($curr_time,'j').'&month='.timestamp_to_user_formated2($curr_time,'n').'&year='.timestamp_to_user_formated2($curr_time,'Y').'">'.timestamp_to_user_formated2($curr_time,'j').'</a>';
