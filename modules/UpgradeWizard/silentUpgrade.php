...
function upgradeDCEFiles($argv,$instanceUpgradePath){
    ...
    foreach($upgradeTheseFiles as $file){
        ...
        if(file_exists($srcFile)){
            ...
            $_GET['REVERT_FILE'] = false;
            if(!class_exists('TemplateConverter')){
                include($argv[7].'templateConverter.php');
            }

            if(is_file($_GET['TEMPLATE_PATH'])){
                TemplateConverter::convertFile($_GET['TEMPLATE_PATH'],$_GET['REVERT_FILE'],$_GET['CONVERT_FILE_ONLY']);
            }
...
if(instanceOrNot($argv,$usage_dce) == constant('DCE_INSTANCE')){
    ...
    //get entry point from destination template
    $dceupgrade_pos = strpos($argv[1], '/DCEUpgrade');
    $newtemplate_path = substr($argv[1], 0, $dceupgrade_pos);
    require_once("{$newtemplate_path}/include/entryPoint.php");
...
//    require_once("{$cwd}/sugar_version.php"); //provides instance version, flavor etc..
    //get sugar version from source template
    require_once($argv[4]."/sugar_version.php"); //provides instance version, flavor etc..
    $isDCEInstance = true;
...
//          copy("{$cwd}/{$sugar_config['upload_dir']}upgrades/temp/su_temp/manifest.php", "{$cwd}/{$sugar_config['upload_dir']}upgrades/patch/{$zip_from_dir}-manifest.php");
            mkdir_recursive("{$argv[3]}/{$sugar_config['upload_dir']}upgrades/patch/");
            copy("$argv[1]/manifest.php", "{$argv[3]}/{$sugar_config['upload_dir']}upgrades/patch/{$zip_from_dir}-manifest.php");
...
//  require_once("{$instanceUpgradePath}/modules/UpgradeWizard/uw_utils.php");
    ...
    if($exitOnConflict == 'yes'){
        ...
        if($customFiles != null){
            ...
            if($existsCustomFile){
                ...
                logThis("There are customized files. Take a look in the upgrade log", $path);
                //die();
            }
            else{
    ...
    if($skipDBUpgrade == 'no'){
        ...
        //include tab controller for later processing.  We instantiate the controller here
        //before any scripts alter the modules during post_install 
        require_once($newtemplate_path.'/modules/MySettings/TabController.php');
        $newTB = new TabController();
        logThis('--- initial system tabs are '.var_export($newTB->get_system_tabs(),true),$path);
    ...
    ////    HANDLE POSTINSTALL SCRIPTS
        ...
        logThis('old modules in module list are '.var_export($oldModuleList,true),$path);
        logThis('new modules in module list are '.var_export($newModuleList,true),$path);
		...
		logThis('final system tabs are '.var_export($tabs,true),$path);
