...
class ACLAction  extends SugarBean{
	...
	*/
	...
	function getUserActions($user_id,$refresh=false, $category='',$type='', $action=''){
        ...
        if (empty($query)) {
            ...
            $query = "(SELECT acl_actions .*, acl_roles_actions.access_override, 1 as user_role
                    ...
                    INNER JOIN acl_roles_actions ON acl_roles_actions.role_id = acl_roles_users.role_id AND acl_roles_actions.action_id = acl_actions.id AND acl_roles_actions.deleted=0
					...
					INNER JOIN acl_roles_actions ON acl_roles_actions.role_id = securitygroups_acl_roles.role_id AND acl_roles_actions.action_id = acl_actions.id AND acl_roles_actions.deleted=0
					WHERE acl_actions.deleted=0 $additional_where )
	...
	function userHasAccess($user_id, $category, $action,$type='module', $is_owner = false, $in_group = false){
		...
		if(ACLAction::getUserAccessLevel($user_id, $category, 'access') < ACL_ALLOW_ENABLED) return false;
		if(empty($_SESSION['ACL'][$user_id][$category][$type][$action])){
			ACLAction::getUserActions($user_id, false);
