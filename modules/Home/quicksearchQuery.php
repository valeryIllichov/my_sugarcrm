    ...
    function query($args) {
        ...
        for($i = 0; $i < count($list_return); $i++) {
            ...
            foreach($args['field_list'] as $field) {
                ...
                if($list_return[$i]->shipping_address_street != '') {
                $list_arr['fields'][$i]['expanded_name'] = $list_return[$i]->name . ' (' . $list_return[$i]->custno_c . ' - ' . $list_return[$i]->shipping_address_street . ', ' . $list_return[$i]->shipping_address_city . ', ' . $list_return[$i]->shipping_address_state .' ' . $list_return[$i]->shipping_address_postalcode .')';
                }
                //hack
                if(isset($args['expanded_name']) && $list_return[$i]->shipping_address_street != '') {
                $list_arr['fields'][$i]['expanded_name'] = $list_return[$i]->name;    
                $list_arr['fields'][$i]['name'] = $list_return[$i]->name . ' (' . $list_return[$i]->custno_c . ' - ' . $list_return[$i]->shipping_address_street . ', ' . $list_return[$i]->shipping_address_city . ', ' . $list_return[$i]->shipping_address_state .' ' . $list_return[$i]->shipping_address_postalcode .')';
                }
                if($module == 'Opportunities' && $field == 'name'){
                    $query = "SELECT opl.pline_id AS pid,pl.description AS pdesc, opl.pcat_id AS pcat, cat.description AS cdesc, opl.pcode_id AS pcode, code.description AS codedesc 
                                        FROM opportunities_product_line opl 
                                        LEFT JOIN dsls_pline_simple pl ON pl.line = opl.pline_id
                                        LEFT JOIN dsls_cat_simple cat ON cat.line = opl.pline_id AND cat.cat = opl.pcat_id
                                        LEFT JOIN dsls_pricecode code ON code.line = opl.pline_id AND code.cat = opl.pcat_id AND code.pc = opl.pcode_id WHERE opl.opportunity_id='" . $list_return[$i]->id . "'";
                    $opportunitypl = $focus->db->query($query);
                    $row = $focus->db->fetchByAssoc($opportunitypl);
                    $line = empty($row['pid']) ? '':$row['pid'].' - '.$row['pdesc'];
                    $cat = empty($row['pcat']) ? '':', '.$row['pcat'].' - '.$row['cdesc'];
                    $pc = empty($row['pcode']) ? '':', '.$row['pcode'].' - '.$row['codedesc'];
                    $row = empty($row) ? 'All' : $line.$cat.$pc;
                    $list_arr['fields'][$i]['expanded_name'] = $list_return[$i]->name .  ' ('.$row.')';
                }
            }
        }
         $GLOBALS['log']->test('quickSearchQuery :'.$list_arr);
