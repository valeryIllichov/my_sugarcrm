...
if(is_array($pages[0]['columns'][1]['dashlets']) && is_array($pages[0]['columns'][o]['dashlets'])) {
    $pages[0]['columns'][0]['dashlets'] = array_merge($pages[0]['columns'][0]['dashlets'], $pages[0]['columns'][1]['dashlets']);
}
$pages[0]['columns'][0]['width'] = '100%';
unset($pages[0]['columns'][1]);
...
$hasUserPreferences = (!isset($pages) || !isset($dashlets)) ? false : true;
...
foreach($pages[$activePage]['columns'] as $colNum => $column) {
    ...
    //echo'<pre>';print_r($dashletsFiles);echo'</pre>';
    //  echo'<pre>';print_r($dashlets);echo'</pre>';
    if(count($column['dashlets']) > 0) {
