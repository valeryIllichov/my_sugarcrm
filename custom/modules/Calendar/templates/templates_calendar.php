...
<?php
	...
	function template_echo_slice_activities(& $args) {
		...
		foreach($args['slice']->acts_arr[$current_user->id] as $act) {
			...
			$extra = "onmouseover=\"return overlib('" . 
					...
					. ", CLOSETEXT, '<img border=0  style=\'margin-left:2px; margin-right: 2px;\' src={$image_path}close.gif>', "
			...
			if($act->sugar_bean->object_name == 'Call') { 
				echo '<td class="monthCalBodyDayIconTd">' . get_image($image_path.'Calls','alt="'.$app_list_strings['call_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"'). '</td> 
						...
						$act->sugar_bean->id . '" class="monthCalBodyDayItemLink">' . $app_list_strings['call_status_dom'][$act->sugar_bean->status] . ': ' . $act->sugar_bean->name;
				if(isset($act->sugar_bean->parent_name)) {
					if(strlen($act->sugar_bean->parent_name) > 0) {
						echo ' ('.$act->sugar_bean->parent_name.')';
					}
				}	
				echo '</a></td>';
				...
				$out = '<td class="monthCalBodyDayIconTd">' . get_image($image_path.'Meetings','alt="'.$app_list_strings['meeting_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"') . '</td>
						...
						$act->sugar_bean->id . '" class="monthCalBodyDayItemLink">' . $app_list_strings['meeting_status_dom'][$act->sugar_bean->status] . ': ' . $act->sugar_bean->name; 
				if(isset($act->sugar_bean->parent_name)) {
					if(strlen($act->sugar_bean->parent_name) > 0) {
						$out .= ' ('.$act->sugar_bean->parent_name.')';
					}
				}		
				$out .= '</a>';
				...
				echo '<td class="monthCalBodyDayIconTd">' .  get_image($image_path.'Tasks','alt="'.$act->sugar_bean->status.': '.$act->sugar_bean->name.'"') . '</td>
						<td class="monthCalBodyDayItemTd" width="100%"><a ' . $extra . ' href="index.php?module=Tasks&action=DetailView&record=' . $act->sugar_bean->id . '" class="monthCalBodyDayItemLink">'.$app_list_strings['task_status_dom'][$fields['STATUS']].': ' . $act->sugar_bean->name . '</a></td>';
			}
			echo '</tr></table><div>';
		}
	}
	...
	function template_calendar(& $args) {
		...
		if(isset($_REQUEST['view']) && $_REQUEST['view'] == 'shared') {
			...
			$tools = '<div align="right"><a href="index.php?module='.$currentModule.'&action='.$action.'&view=shared" class="chartToolsLink">&nbsp;<a href="javascript: toggleDisplay(\'shared_cal_edit\');" class="chartToolsLink">'.get_image($image_path.'edit', 'alt="'.$current_module_strings['LBL_EDIT'].'"  border="0"  align="absmiddle"').'&nbsp;'.$current_module_strings['LBL_EDIT'].'</a></div>';
			...
			echo "	</select></td>
				<td><a onclick=\"up('shared_ids');\">".get_image($image_path.'uparrow_big', 'border="0" style="margin-bottom: 1px;" alt="'.$app_strings['LBL_SORT'].'"')."</a><br>
